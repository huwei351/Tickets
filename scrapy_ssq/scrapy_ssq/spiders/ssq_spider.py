...
class myspider(scrapy.Spider):
    ...
    def parse(self, response):
        page = BeautifulSoup(response.text, 'lxml').find('div', class_='page-box house-lst-page-box')#找出每个区域最大的页数，然后遍历
    ...
    def get_message(self,response):
        ...
        for li in house_list:
            ...
            item['area'] = re.findall(r'(\d+)', li.find('span', class_='meters').get_text().replace('&nbsp', ''))[0]  # 将面积数据提取出来
