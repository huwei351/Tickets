...
#define MAX_SECTION_CONTEXT_BUF_SIZE  256
/*
...
class Config
{
    ...
    protected:
        ...
        typedef std::map<std::string, MapKeyValue* >::iterator mapsi; // Section map iterator
        typedef std::map<std::string, MapKeyValue* >::const_iterator mapsci; // Section map const iterator

        std::string m_Delimiter;  //!< separator between key and value
        std::string m_Comment;    //!< separator between value and comments
        std::string m_Section; //!< default section name
        MapKeyValue* m_Contents;  //!< extracted keys and values
        MapSection m_mapSection;  //!< collection of extracted sections

        // Methods
    public:

        Config(std::string filename, std::string delimiter = "=", std::string comment = "#", std::string section = "default");
        Config();
        ...
        // Add a section if not exist
        bool AddSection(const std::string section);

        // Change to section
        bool ChangeSection(const std::string section);
    ...
    private:
        ...
        bool IsCommentLine(const char* szLine);
        bool IsEmptyLine(const char* szLine);
        bool IsNewSection(const char* szLine);
        bool IsKeyValueLine(const char* szLine);
        bool GetNewSectionContext(const char* szLine, std::string& strNewSectionContext);
        static void Trim(std::string& inout_s);
        static void Split(std::string str, const char* delimiter, std::vector<std::string>& tokens);
        // Exception types
    public:
...
T Config::Read(const std::string& key) const
{
    ...
    { return string_as_T<T> ("KEY NOT FOUND"); }
